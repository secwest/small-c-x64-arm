/* sld_win_arm64.c - Simple PE Linker for Small-C (Windows ARM64) */
/* Creates Windows PE executables from COFF object files for ARM64 */
...
int encode_branch26(int offset) {
    return (offset >> 2) & 0x3FFFFFF;
}

int encode_branch19(int offset) {
    return (offset >> 2) & 0x7FFFF;
}

int encode_adrp_imm(int offset) {
    return ((offset >> 12) & 0x1FFFFF);
}
...
int read_coff(char *filename, char *buf) {
    ...
    if (machine != IMAGE_FILE_MACHINE_ARM64) {
        puts("Error: Not an ARM64 COFF file\n");
        return -1;
    }
...
void apply_relocations() {
    ...
    for (i = 0; i < reloc_count; i++) {
        ...
        /* Current address */
        int pc = IMAGE_BASE + section_rvas[section] + offset;
        ...
        case IMAGE_REL_ARM64_BRANCH26:
            /* BL/B instruction - 26-bit offset */
            *instr = (*instr & 0xFC000000) | encode_branch26(target - pc);
            break;
            
        case IMAGE_REL_ARM64_BRANCH19:
            /* Conditional branch - 19-bit offset */
            *instr = (*instr & 0xFF00001F) | (encode_branch19(target - pc) << 5);
            break;
            
        case IMAGE_REL_ARM64_PAGEBASE_REL21:
            /* ADRP instruction - page base */
            {
        ...
        case IMAGE_REL_ARM64_PAGEOFFSET_12L:
            /* LDR/STR immediate - 12-bit page offset */
            {
                int shift = (*instr >> 30) & 3; /* Size of load/store */
                int offset = (target & 0xFFF) >> shift;
                *instr = (*instr & 0xFFC003FF) | (offset << 10);
            }
            break;
        }
    }
...
int write_pe(char *filename) {
    ...
    write_u16(coff, IMAGE_FILE_MACHINE_ARM64);
    ...
    write_u16(opt + 40, 10); /* OS major - Windows 10 */
    write_u16(opt + 42, 0); /* OS minor */
    write_u16(opt + 48, 10); /* Subsystem major */
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        puts("Usage: sld_win_arm64 [-o output.exe] file1.obj file2.obj ...\n");
        return 1;
    }
